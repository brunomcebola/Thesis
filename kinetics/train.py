...
LOGITS_I3D_CHECKPOINT = f"checkpoints/{TYPE}_{BASE}_logits/model.ckpt"
MIXED_5C_CHECKPOINT = f"checkpoints/{TYPE}_{BASE}_mixed_5c/model.ckpt"
TUNED_I3D_CHECKPOINT = f"checkpoints/{TYPE}_CLS_{DATA}_logits/model.ckpt"
...
def ensure_mixed_5c_checkpoint():
    ...
    model(
        tf.convert_to_tensor(
            np.random.rand(1, 64, 224, 224, 3 if TYPE == "rgb" else 2), dtype=tf.float32
        )
    )
...
def save_model(model, cls, train_accuracies, val_accuracies, train_losses, val_losses):
    ...
    with open(destination.replace(".ckpt", ".txt"), "w", encoding="utf-8") as f:
        f.write("\n".join(cls.keys()))
