...
_SAMPLES_PATH = "npy/color/swipe_card"
...
def main():
    ...
    classes = [
        x.strip()
        for x in open(
            os.path.join(_CHECKPOINTS_PATH, "rgb_3_logits", "model.txt"), encoding="utf-8"
        )
    ]
    ...
    tf.train.Checkpoint(model=rgb_model).restore(
        os.path.join(_CHECKPOINTS_PATH, "rgb_3_logits", "model.ckpt")
    ).expect_partial()
    ...
    tf.train.Checkpoint(model=flow_model).restore(
        os.path.join(_CHECKPOINTS_PATH, "flow_3_logits", "model.ckpt")
    ).expect_partial()
    ...
    for folder in tqdm.tqdm(
        ...
        joint_label = classes[joint_index]
        ...
        del rgb_samples, rgb_logits, rgb_sample
        del flow_samples, flow_logits, flow_sample
        gc.collect()
        tf.keras.backend.clear_session()
