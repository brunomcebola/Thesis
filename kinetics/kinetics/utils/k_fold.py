...
def finetune(
    ...
    npy_samples_paths = np.array(npy_samples_paths)
    ...
    for kfold, (train_index, val_index) in enumerate(skf.split(npy_samples_paths, labels)):
        ...
        # Create datasets
        train_samples, train_labels = npy_samples_paths[train_index], labels[train_index]
        val_samples, val_labels = npy_samples_paths[val_index], labels[val_index]
        ...
        for epoch in tqdm(range(epochs), desc=f"Training K Fold {kfold + 1}"):
            ...
            train_scores[kfold].append(train_metric)

            # Validate model
            validation_loss, validation_metric = validation_step(
                model,
                validation_dataset,
                loss_fn_generator(),
                score_generator(),
            ...
            validation_scores[kfold].append(validation_metric)
    ...
    print(
        f"Best model found at K Fold {best_kfold} in Epoch {best_epoch} with Metric {best_validation_metric}"
    )
