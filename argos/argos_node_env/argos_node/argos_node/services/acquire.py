...
class AcquireService(base.Service):
    ...
    _LOG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "acquire.log")
    ...
    def run(self) -> None:
        ...
        try:
            ...
            printer.print_info("New acquisition session started!\n")
            ...
            printer.print_info("To stop data acquisition press Ctrl + C!\n")

            self._stream_signals.error.wait()

            printer.print_error("Error in one of the camera streams.")

            self._logger.error("Error in one of the camera streams.")

        except KeyboardInterrupt:
            printer.print_info("\rCtrl + C pressed\n")
        ...
        printer.print_info(stats)

        self._logger.info("Acquisition session finished.\n")

        printer.print_info("Acquire service terminated!\n")
